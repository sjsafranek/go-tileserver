...
// NewTileServerMux creates TileServeMux object.
func NewTileServerMux(cacheFile string) *TileServerMux {
	t := TileServerMux{}
	t.lmp = NewLayerMultiplex()
	t.m = NewTileCache(cacheFile)
...
func (self *TileServerMux) AddMapnikLayer(layerName string, stylesheet string) error {
...
func (self *TileServerMux) hasLayer(layerName string) bool {
	for k := range self.lmp.layerChans {
		if k == layerName {
			return true
		}
...
// func (self *TileServerMux) GetTileLayer(w http.ResponseWriter, r *http.Request) {
// 	start := time.Now()
// 	vars := mux.Vars(r)
// 	lyr := vars["lyr"]
...
func (self *TileServerMux) NewTileLayer(w http.ResponseWriter, r *http.Request) {
	start := time.Now()
...
func (self *TileServerMux) ServeTileRequest(w http.ResponseWriter, r *http.Request) {
...
func (self *TileServerMux) TMSTileMaps(w http.ResponseWriter, r *http.Request) {
	start := time.Now()
	var layers []string
	for k := range self.lmp.layerChans {
...
func (self *TileServerMux) TMSTileMap(w http.ResponseWriter, r *http.Request) {
	start := time.Now()
	vars := mux.Vars(r)
	lyr := vars["lyr"]
	if _, ok := self.lmp.layerChans[lyr]; !ok {
...
func (self *TileServerMux) ServerProfileHandler(w http.ResponseWriter, r *http.Request) {
	ServerProfileHandler(self.startTime, w, r)
}

// TileLayersHandler returns list of tiles.
func (self *TileServerMux) TileLayersHandler(w http.ResponseWriter, r *http.Request) {
	start := time.Now()
	var keys []string
	for k := range self.lmp.layerChans {
		keys = append(keys, k)
