...
func GetTileNames(minlat, maxlat, minlng, maxlng float64, z int) []xyz {
	...
	// CREATE MASTER IMAGE
	NUM_COLS = (ll_tile_y + 2) - (ur_tile_y - 2)
	MIN_Y = ll_tile_y - 2
	MAX_Y = ur_tile_y + 2
	NUM_ROWS = (ur_tile_x + 2) - (ll_tile_x - 2)
	MIN_X = ll_tile_x - 2
	MAX_X = ur_tile_x + 2
...
func clipImage(inImage image.Image) image.Image {
	...
	// draw.Draw(outImage, bounds, inImage, image.Point{x, y}, draw.Src)
